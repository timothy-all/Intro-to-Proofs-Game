...
Let's use the laws of logic to prove that the ***Contrapositive*** of an implication is logically equivalent to the original implication.
"
...
Statement Contrapositive {P Q : Prop} : (P → Q) ↔ (¬ Q → ¬ P) := by
  ...
  Hint "Here's where our newly unlocked theorem `Not_not` can be handy. 🔍 Check out the entry for `Not_not` in the **Theorems** tab."
  rw[Not_not]
